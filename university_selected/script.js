...
universityOptions.forEach(option => {
  ...
  const universityOptions = document.querySelectorAll('.university-option');
const selectedUniversityButton = document.getElementById('selectedUniversity');
});
...
// Toggle the dropdown when the university button is clicked
universityButton.addEventListener('click', (e) => {
  e.stopPropagation();
  universityDropdownMenu.classList.toggle('show');
});

// Close the dropdown if the user clicks outside of it
document.addEventListener('click', (e) => {
  if (!universityDropdownMenu.contains(e.target) && e.target !== universityButton) {
    universityDropdownMenu.classList.remove('show');
  }
});
