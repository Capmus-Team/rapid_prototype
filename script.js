...
// Handle browser history (back button, etc.)
...
// If the URL has a #post=ID, open that post on load
if (window.location.hash.startsWith("#post=")) {
  const id = parseInt(window.location.hash.replace("#post=", ""), 10);
  const post = posts.find(p => p.id === id);
  if (post) {
...
universityOptions.forEach(option => {
  option.addEventListener('click', () => {
    ...
    universityOptions.forEach(opt => opt.classList.remove('active'));
...
function updateUniversityBarArrow() {
  if (universityBarEl.scrollWidth > universityBarEl.clientWidth) {
    ...
    universityScrollArrowEl.classList.remove('show');
  }
}
universityBarEl.addEventListener('scroll', () => {
  // (Optional: logic to hide arrow if fully scrolled)
});
universityScrollArrowEl.addEventListener('click', () => {
